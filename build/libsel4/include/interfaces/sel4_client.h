 ...
 */
struct seL4_ARM_VSpace_Read_Word {
	int error;
	seL4_Word value;
};
typedef struct seL4_ARM_VSpace_Read_Word seL4_ARM_VSpace_Read_Word_t;
...
/**
 * @xmlonly <manual name="Read Word" label="aarch64_vspace_read_word"/> @endxmlonly
 * @brief @xmlonly Read a word from some address in a vspace. @endxmlonly
 * 
 * @xmlonly
 ...
 * 
 * @param[in] _service Capability to the top level translation table being operated on.
 * @param[in] vaddr Address to read from 
 ...
 * @retval seL4_RangeError The specified range crosses a page boundary. 
 */
LIBSEL4_INLINE seL4_ARM_VSpace_Read_Word_t
seL4_ARM_VSpace_Read_Word(seL4_ARM_VSpace _service, seL4_Word vaddr)
{
	seL4_ARM_VSpace_Read_Word_t result;
	seL4_MessageInfo_t tag = seL4_MessageInfo_new(ARMVSpaceRead_Word, 0, 0, 1);
	seL4_MessageInfo_t output_tag;
	seL4_Word mr0;
	seL4_Word mr1;
	seL4_Word mr2;
...
#if defined(CONFIG_DEBUG_BUILD)
/**
 * @xmlonly <manual name="Write Word" label="aarch64_vspace_write_word"/> @endxmlonly
 * @brief @xmlonly Write a word to some address in a vspace. @endxmlonly
 * 
 * @xmlonly
 * The <texttt text="_service"/> is not assigned to an ASID pool.
 * @endxmlonly
 ...
 * @param[in] _service Capability to the top level translation table being operated on.
 * @param[in] vaddr Address to read from 
 * @param[in] value Value to write to the address 
 * @return @xmlonly <errorenumdesc/> @endxmlonly
 * @retval seL4_FailedLookup The  @xmlonly <texttt text="_service"/> @endxmlonly  is not assigned to an ASID pool. 
 * @retval seL4_IllegalOperation The  @xmlonly <texttt text="_service"/> @endxmlonly  is a CPtr to a capability of the wrong type.
 * Or,  @xmlonly <texttt text="end"/> @endxmlonly  is in the kernel virtual address range. 
 ...
 */
LIBSEL4_INLINE seL4_Error
seL4_ARM_VSpace_Write_Word(seL4_ARM_VSpace _service, seL4_Word vaddr, seL4_Word value)
{
	seL4_Error result;
	seL4_MessageInfo_t tag = seL4_MessageInfo_new(ARMVSpaceWrite_Word, 0, 0, 2);
	seL4_MessageInfo_t output_tag;
	seL4_Word mr0;
	seL4_Word mr1;
	seL4_Word mr2;
